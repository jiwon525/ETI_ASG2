...
func deleteClass(ClassID int) string {
	//set up url
	url := classURL + "?classid=" + strconv.Itoa(ClassID) + "&key" + key
...
func searchClasses(ModuleID string) (string, []Classes) {
	//set up url
	url := "http://localhost:9101/api/v1/class?ModuleCode=" + ModuleID + "&key" + key
	// Create request
	response, err := http.Get(url)
	var c []Classes
	var errMsg string
...
func searchClassesMenu() {
	...
	fmt.Scanln(&c.ModuleCode)
	fmt.Println("--------------------------")
	errMsg, cc := searchClasses(c.ModuleCode)
